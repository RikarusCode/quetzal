export const BUILD_ID='quetzal-8v4-e9fc9cf506ad11db-gpsp-8d268a6-mul_poke-v2';
...
export function encodePacket(flags,bytes,to=65535){
  if(!validGamePacket(bytes)||!Number.isInteger(flags)||(flags&~5)!==0||!([0,1,2,3,65535].includes(to)))throw Error('Unsupported emulator packet.');
  const frame=new Uint8Array(32);frame[0]=RELAY_VERSION;frame[1]=flags;frame[3]=to===65535?255:to;frame.set(bytes,8);return frame.buffer;
}
export function decodePacket(buffer){
  ...
  if(frame[0]!==RELAY_VERSION||(frame[1]&~5)!==0||frame[2]>=MAX_PLAYERS||!(frame[3]<MAX_PLAYERS||frame[3]===255)||!validGamePacket(frame.subarray(8)))throw Error('Invalid relay packet.');
  return {flags:frame[1],from:frame[2],to:frame[3]===255?65535:frame[3],sequence:new DataView(buffer).getUint32(4),bytes:frame.slice(8)};
}
